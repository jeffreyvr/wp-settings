...
class WPSettings
{
	...
	{
		if ( $this->parent_slug ) {
			\add_submenu_page( $this->parent_slug, $this->title, $this->get_menu_title(), $this->capability, $this->slug, [
				$this,
				'render',
			], $this->menu_position );
		} else {
			\add_menu_page( $this->title, $this->get_menu_title(), $this->capability, $this->slug, [
				$this,
				'render',
			], $this->menu_icon, $this->menu_position );
		}
	}
	...
	public function make()
	{
		...
		add_filter( "option_{$this->option_name}", [ $this, 'set_default_options' ], 9999 );
	}
	...
	public function save()
	{
		...
		foreach ( $this->get_active_tab()->get_active_sections() as $section ) {
			foreach ( $section->options as $option ) {
				$value = ( new Dot( $submitted_options ) )->get( $option->implementation->get_option_key_path() );
	...
	public function set_default_options( $db_options )
	{
		static $cache = [];
		...
		$cache_key = md5( serialize( $db_options ) );
		...
		/**
		 * @var $tab     \Jeffreyvr\WPSettings\Tab
		 * @var $section \Jeffreyvr\WPSettings\Section
		 * @var $option  \Jeffreyvr\WPSettings\Option
		 */

		foreach ( $this->tabs as $tab ) {
			$keys = [];

			if ( $tab->is_option_level() ) {
				$keys[] = str_replace( '-', '_', $tab->slug );
			}

			foreach ( $tab->sections as $section ) {
				if ( $section->is_option_level() ) {
					$keys[] = str_replace( '-', '_', $section->slug );
				}

				foreach ( $section->options as $option ) {
					$default = $option->get_arg( 'default' );
					...
					unset( $value, $default, $keys[ $last_key_index ] );
				}
			}
		}

		$cache[ $cache_key ] = $db_options;
